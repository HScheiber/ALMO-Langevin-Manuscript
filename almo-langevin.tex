...
\begin{abstract} 
%The utility of density functional theory based on compact localized molecular orbitals in \emph{ab initio} molecular dynamics. To overcome limitations of existing linear-scaling methods
%%in conjunction with a modified Langevin equation of motion 
%to create a linear-scaling \emph{ab initio} molecular dynamics method for weakly coupled molecular systems. 
%To overcome %which retains sufficient accuracy to generate accurate dynamical properties in water. 
%%Strictly localized 
%%Low-cost linear-scaling ab initio molecular dynamics for weakly-interacting systems is developed 
%%for weakly-interacting systems. 
%%High efficiency of the methodology is achieved without sacrificing its accuracy with a combination of two techniques: (1) on-the-fly calculation of approximate forces without lengthy self-consistent optimization of localized orbitals and (2) integration of a modified Langevin equation of motion that is fine-tuned to retain stable and correct dynamics even with imperfect forces. 
...
%Application of the method to liquid water demonstrates that the new AIMD method enables simulations of molecular systems on previously inaccessible time and length scales. 
...
%Systematic inaccuracies in the calculation of force using this method are balanced by a weighted version of the Langevin equation. 
...
$\delta f_{i\alpha} (t)$ can be reduced to zero \emph{systematically} by increasing $R_c$ and decreasing $\epsilon_{\text{SCF}}$. %$\epsilon_{\text{SCF}}$ is chosen such that the electronic ground state is considered sufficiently close to the Born-Oppenheimer state for values less than $\epsilon_{\text{SCF}}$.
...
%RZK: "G_OO -> g_OO", "Intensity (au)" -> "Intensity", remove "DFT" in two legend labels 
...
%at $\SI{302}{\K}$ and  --- RZK: It is customary to report TARGET temperature, not the acutal average temperature. 
%with the Langevin thermostat with $\gamma = \SI{e-3}{\per\fs}$. 
%All other system parameters were set identical to the ALMO system.

%RZK: 5K deviation from the target temperature is considered insignificant. Even with perfect forces, the actual temperature deviates from the target temperature by 5 K. This shows that our simulations are still too short to get the correct average. This is fine though.
...
%RZK: Change panels: Remove panel (a), old panel (c) becomes new panel (a), old (b) -> new (b), old (d) -> new (c). The caption and the text has aready been re-written. 
...
%RZK: (a) $R_{c} = 1.6$ vdWR and $\epsilon_{\text{SCF}} = 10^{-2}$~a.u. relative to the fully converged ALMO reference, 
...
For liquid water, strict localization of ALMOs lowers the crossover point between ALMO AIMD and cubically scaling methods to 256 molecules -- length scales routinely accessible with AIMD today. It is important to emphasize that the comparison is performed for a three-dimensional condensed phase system described with a large diffuse basis set -- a particularly challenging case for all LS methods.
...
%RZK: On figure remove 'DFT', 'Wall-clock time' -> 'Wall time'. Decrease the height of the figure (the y-axis label is now shorter).
}
\end{figure}
...
%RZK: On figure 'Wall-clock time' -> 'Wall time', legend - increase font size, decrease the height of the figure(the y-axis label is now shorter). This figure will be black-and-white in the printed version and will have color online: replace magenta cross with another "solid" symbol such as inverted triangle.
}
\end{figure}
...
The developed methods will have a significant impact on modeling of large heterogeneous molecular systems (e.g. interfaces, nuclei) making completely new phenomena accessible to AIMD. 
...
%RZK: journal abbreviation in Ref.  a:almo-ls
\bibliography{almo-langevin}

%RZK: resolve SI problem
\else
