...
\emph{ab initio} molecular dynamics (AIMD)~\cite{b:aimd} has become an important tool for computational studies of weakly bonded ensembles of molecules such as gas-phase clusters, molecular and ionic liquids, solutions, and molecular solids. [of gas-phase molecules and condensed-phase systems.] 
Unfortunately, the computational cost of AIMD based on the conventional Kohn-Sham (KS) DFT grows cubically with the number of atoms in the system and severely limits the \emph{length scale} accessible by AIMD. 
In an attempt to address this issue, substantial efforts have been directed to the development of linear scaling (LS) DFT methods for the on-the-fly calculation of forces.
...
The steep scaling of the conventional KS DFT originates from the delocalized nature of the canonical KS orbitals, which tend to delocalize over all atoms in the system, with an alternative set of electronic descriptors. All LS methods must A number of such methods explore the natural sparsity of the one-electron density matrix (DM)~\cite{a:ls-kohn-1996,a:ls-rev-1999} and are capable of yielding LS for large systems.~\cite{a:linscale3,a:lee-yang-1996,a:ls-scuseria-1997,a:ls-manolopoulos-1998,a:ls-helgaker-2001,a:ls-niklasson-2003,a:curvy2,a:ls-dm-sign} 
...
Here, we introduce a new low-cost linear scaling AIMD method for weakly-interacting molecular systems based on the recently developed absolutely localized molecular orbitals (ALMO) DFT approach~\cite{a:almo-ls}. ALMO DFT exploits non-orthogonal molecular orbitals (MOs) in a two-step self-consistent filed (SCF) approach that is designed to avoid the usual convergence issues~\cite{a:ls-mauri-galli-car-1993,a:ls-ordejon-1995,a:ls-fattebert-2004,a:ls-rev-1999} and .
...
\begin{figure}
\includegraphics[trim={0.5cm 0.7cm 0.5cm 0cm},clip,width=8cm]{DeltaForceComparison_with_ACF}
...
\begin{figure}
\includegraphics[trim={0.1cm 0cm 0cm 0cm},clip,width=8cm]{DeltaForceComparison_ALMO_SCF}
...
\begin{figure}
\includegraphics[trim={1.9cm 0.4cm 3.5cm 1.7cm},clip,width=8cm]{strongscaling_linear}
...
\begin{figure}
\includegraphics[trim={2.8cm 0.4cm 3.5cm 1.2cm},clip,width=8cm]{strongscaling_log}
