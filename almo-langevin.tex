...
\begin{abstract} 
We coupled a robust method for constructing compact localized orbitals within the framework of Kohn-Sham density functional theory with a tunable Langevin equation of motion to obviate the commonly employed optimization of the one-electron density matrix and to create an efficient \emph{ab initio} molecular dynamics method for weakly-interacting systems. 
%An application to liquid water demonstrates that the linear scaling complexity and low computational overhead of the developed method make it suitable for accurate first-principle simulation of a wide range of systems: from routine medium-size systems to previously inaccessible length scales.
%well suited for routine first-principle modeling as well as 
%for a wide range of length scales -- from routinely accessible to previously inaccessible.
%In contrast to linear-scaling first-principle simulation methods that exploit the locality of the one-electron density matrix we coupled a robust method for constructing compact localized orbitals with a modified Langevin equation of motion to create an efficient \emph{ab initio} molecular dynamics method for weakly-interacting systems.
%A robust method for constructing compact localized orbitals in density functional theory is coupled with a tunable Langevin equation of motion to create low-cost linear-scaling \emph{ab initio} molecular dynamics for weakly-interacting systems. 
An application to liquid water demonstrates that the low computational overhead of the developed method makes it well-suited for routine simulations whereas its linear scaling complexity allows to extend first-principle dynamical studies of molecular systems to previously inaccessible length scales. 
%
%%%A robust method for constructing compact localized orbitals in density functional theory enables us to create low-cost linear-scaling \emph{ab initio} molecular dynamics for weakly-interacting systems. 
%%%High efficiency of simulations is achieved by calculating the forces approximately without lengthy optimization of localized orbitals and by adjusting the Langevin equation of motion to stabilize the dynamics with the imperfect forces. 
%%%An application to liquid water demonstrates that a low computational overhead of this procedure makes it suitable for routine medium-size simulations whereas its linear scaling cost allows to extend accurate first-principle molecular dynamics 
%%%%[RZK: of molecular systems] 
%%%to previously inaccessible length scales. 
%%%% RZK: no DM, only orbitals. Make it sound like a theory development not implementation focused.
\end{abstract}
...
Most LS methods~\cite{a:ls-rev-1999, a:ls-rev-2012, Kussmann2013, Aarons2016, optimal-basis-too} explore the natural locality of the one-electron density matrix (DM). 
However, with accurate multifunction basis sets the DM DFT becomes advantageous only for impractically large systems~\cite{a:ls-dm-sign, Arita2014, a:ls-rev-2012, a:ls-almo}.
...
Despite becoming the most popular approach to LS DFT the efficiency of these methods is hampered by the costly optimization of both the contracted orbitals and the DM~\cite{convergence-gilan}. 
%
...
%
From this point of view, a direct variation of molecular orbitals that are strictly localized within predefined regions is preferable because LS can be achieved without costly DM optimization and with significantly fewer variables. %especially in the case of large accurate basis sets. 
Advantages of the orbitals-only DFT are especially pronounced in accurate calculations that require many basis functions per atom.
...
Unfortunately, the development of promising orbital-based LS methods has been all but abandoned~\cite{a:review1999, a:weitao-yang-2013,japanese-guys} because of the inherently difficult optimization of localized orbitals~\cite{a:ls-mauri-galli-car-1993,a:ls-ordejon-1995,a:ls-fattebert-2004,a:ls-rev-1999,a:weitao-yang-2013,japanese-guys-citedinmy2013paper}. 
...
%Thus, the high computational overhead of existing LS DFT methods restrict their use in [dynamical] AIMD simulations to very short \emph{time scales}, systems of low dimensions, and low-quality minimal basis sets~\cite{a:ls-dm-sign, RZK:10.7567/JJAP.55.1102B1,Hine2011, Bowler2010}. 
...
However, the same approach is readily applicable to systems of strongly-interacting fragments that do not form strong covalent bonds such as ionic salts, liquids, and semiconductors. 
...
The challenge of adopting ALMO DFT for dynamical simulations arises from the slightly nonvariational character of the localized orbitals. First, while ALMOs are variationally optimized in both SCF stages, the occupied subspace defined in the first stage must remain fixed during the second stage to ensure convergence. Second, neglected electron transfer effects can suddenly become active in the course of a dynamical simulation when a neighboring molecule passes the localization threshold $R_{c}$. In addition to these issues specific to ALMO DFT, the variational optimization in any AIMD method is never truly complete in practice
%[exact convergence is never achieved in practical calculation] 
and interrupted once the maximum norm of the gradient of the energy with respect to the electronic descriptors drops below small but nevertheless finite convergence threshold $\epsilon_{\text{SCF}}$. These nonvariational errors do not affect the accuracy of static ALMO DFT calculations, geometry optimization, and Monte-Carlo simulations. Unfortunately they tend to accumulate in molecular dynamics trajectories leading to non-physical sampling and eventual failure. 
Traditional strategies to cope with these problems are computationally expensive and include computing the nonvariational contribution to the forces via a variational coupled-perturbed procedure~\cite{Kussmann2013}, increasing $R_c$, and decreasing $\epsilon_{\text{SCF}}$. 
...
Thus, the inherently stochastic approach presented here does not aim to produce fully time-reversible dynamics for atomic nuclei. 
However, with $\gamma=0$, the nuclei will follow time-reversible Newtonian trajectories sufficiently closely as long as partially optimized ALMOs remain close to the ground state and keep $\Delta$ small. 
%
%(for systems with slight discontinuities in the potential energy surfaces)
%
Although not a required component of ALMO AIMD, the predictor part of the Kolafa scheme~\cite{kolafa2003} adopted to localized orbitals was found to generate a highly accurate initial ALMOs in both SCF stages, which can be brought close to the ground state with just a few SCF steps. 
...
%RZK: time-reversibility must be mentioned or even addressed
...
%RZK: number of atoms is 98,304 with very accurate basis set.
...
within reasonable wall-clock time using only moderate number of compute cores -- an unprecedented feat for AIMD considering that accurate molecular orbitals and the idempotent DM are computed on each step.
