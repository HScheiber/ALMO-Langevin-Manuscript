...
\documentclass[10pt,aps,prb,twocolumn,amsmath,amssymb,superscriptaddress,longbibliography]{revtex4-1}
...
AIMD provides a great advantage over traditional predefined-potential molecular dynamics in that it calculates its PES on-the-fly, rather than needing to be laboriously optimized to the specific system of interest beforehand. 
...
A number of such alternative methods explore the natural sparsity of the one-electron density matrix (DM)~\cite{a:ls-kohn-1996,a:ls-rev-1999} and are capable of yielding linear scaling (LS) for large systems.~\cite{a:linscale3,a:lee-yang-1996,a:ls-scuseria-1997,a:ls-manolopoulos-1998,a:ls-helgaker-2001,a:ls-niklasson-2003,a:curvy2,a:ls-dm-sign} 
...
We also introduce an easy to follow algorithmic approach to optimize a system for generating correct dynamical behaviour.
...
We found that a value of $\gamma_L$ between \SI{1e-5}{\per\fs} and \SI{1e-3}{\per\fs} was optimal for the purposes of the research presented here.
...
A trajectory with inaccurate forces, \mbox{$\gamma_L = \SI{0}{\per\fs}$}, and optimized $\Delta$ should have no long term drift in the total energy of the system. Short term energy fluctuations should be expected, and can later be ``smoothed out'' by setting a finite $\gamma_L$.
...
In practice, this method for estimating $\Delta$ is not particularly accurate due to rapidly oscillating correlations between sequential values of $R^{\Delta}_{i\alpha} (t)$, the ACF of which is not possible to accurately integrate without running a reference trajectory with extremely short time-steps (see figure~\ref{fig:randomforce}).

Once $\gamma_L$ and $\Delta$ are both optimized, approximate forces can be used to perform long MD simulations efficiently.
\bigskip

\begin{figure}
\includegraphics[trim={2.2cm 0.5cm 3cm 0.2cm},clip,width=8cm]{125mol_ALMO_Early_Random_force.eps}
\caption{\label{fig:randomforce}The average ``Random force'' along a water molecule dimension $\langle R^{\Delta}_{i\alpha} \rangle (t)$ calculated as the difference between the fully converged Born-Oppenheimer forces and unconverged ALMO DFT forces, where $\epsilon_{err} = \num{1e-2}$. 
(a) Plot of $\langle R^{\Delta}_{i\alpha} \rangle (t)$ as a function of time. 
Here it is clear that Eq.\ (\ref{eq:stochastic2}) is satisfied. 
(b) Autocovariance function of $\langle R^{\Delta}_{i\alpha} \rangle (t)$. 
The ACF is clearly not a delta-function, and is characterized by rapid oscillation between positive and negative correlation. 
...
Calculations were performed with the PBE $\mathrm{E_{XC}}$ functional and the TZV2P basis set on 128 water molecules.}
\end{figure}
...
For the dynamical properties calculations using ALMO DFT calculation methods, a periodic cell containing 128 water molecules was simulated for $\SI{40}{\ns}$ at a constant temperature of $\SI{298}{\K}$ and a constant density of $\SI{0.9966}{\g\per\cm^{3}}$. 
The system was thermostated using the Langevin ensemble (eq.\ \ref{eq:langevin2}) where $\gamma_L$ was set to $\SI{1e-3}{\per\fs}$, and $\Delta$ was optimized to the order of $10^{-5}$ - $10^{-4}\ \mathrm{fs^{-1}}$. 
Note that for timing benchmarks, $\Delta$ was not optimized. 
This should not have a perceptible effect on calculation time.
...
How $\gamma$, $\gamma_L$, and $\gamma_{\Delta}$ are related to GAMMA, NOISY\_GAMMA, and SHADOW\_GAMMA in CP2K?

\begin{align}
\label{eq:rels}
GAMMA + NOISY\_GAMMA &= \gamma_L + \gamma_{\Delta} = \gamma \nonumber \\
GAMMA + SHADOW\_GAMMA &= \gamma_L = \gamma - \gamma_{\Delta} \nonumber
\end{align}
%
One set of useful expressions
%
\begin{align}
\gamma_{\Delta} &= NOISY\_GAMMA - SHADOW\_GAMMA \nonumber \\
\gamma_L &= GAMMA + SHADOW\_GAMMA \nonumber \\
\gamma &= GAMMA + NOISY\_GAMMA \nonumber
\end{align}
%
%and another one
%%
...
%
\begin{itemize}
\item The code designed so that any effect can be achieved with positive GAMMAs.
\item Situation 1. NVT with perfect forces. Set NOISY and SHADOW to zeros to produce balanced canonical distributions. Large GAMMA adds lots of noise but leads to fast equilibration. Small GAMMA -- approach NVE. 
\item Situation 2. NVT with imperfect forces. Use either NOISY or SHADOW but not both. The value of GAMMA is chosen as in the previous situation. A. Set SHADOW to finite value to get excessive damping. B. Set NOISY to get excessive stochastic boost.
\end{itemize}

Integrator implemented in \$CP2K/src/motion/integrator.F eqns (19) and (20) from http://dx.doi.org/10.1080/0026897031000108113. Another integrator can be implemented using Eq (15.5.17) from the book of Tuckerman.
